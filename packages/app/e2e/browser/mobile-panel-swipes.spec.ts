  ...
  try {
    ...
    const gate = await page.evaluate(() => {
      ...
      return {
        ...
        rulePresent: rules.some((rule) => rule.cssText.includes("--paseo-viewport-compensation")),
      };
    });
...
test("ios-standalone viewport compensation activates for installed iOS PWAs", async ({ page }) => {
  ...
  try {
    await gotoWorkspace(page, seeded.workspaceId);
    // Inject a nonzero compensation through the runtime variable, then
    // confirm the compensated rule wins for html, body, and #root.
    const measured = await page.evaluate(() => {
      document.documentElement.style.setProperty("--paseo-viewport-compensation", "60px");
