...
test.describe("provider usage tooltip", () => {
  test("fetches usage when the context tooltip opens and renders the active provider", async ({
    ...
    try {
      ...
      await expect(page.locator("#overlay-root").getByText("42%")).toBeVisible();
    } finally {
  ...
  test("refreshes usage again each time the tooltip is shown", async ({ page }) => {
    ...
    try {
      ...
      await meter.hover();
      await usageFixture.waitForRequestCount(1);
      await expect(page.locator("#overlay-root").getByText("41%")).toBeVisible({ timeout: 10_000 });
      ...
      await expect(page.locator("#overlay-root").getByText("64%")).toBeVisible();
    } finally {
      await session.cleanup();
    }
  });
