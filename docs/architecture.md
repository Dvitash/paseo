...
- `HostRuntimeController` manages saved host connections, reconnection, and per-host runtime state. Direct TCP and relay connections use the ordinary client transport; desktop socket, pipe, and SSH connections cross one Electron-owned transport boundary. SSH only tunnels to an already-running daemon. Returning to the foreground verifies each host independently with a fresh correlated ping — bounded by the client's `ensureConnectedHealthCheckTimeoutMs` (default 2s) — before any refresh is issued, and retires a transport that fails. A network-online event triggers that same verification but never counts as proof the daemon is reachable.
