...
Read-only Pi sessions (Side) cannot use the adapter: they launch with `--no-extensions`, so only Paseo's generated guard extension loads. For those sessions the extension itself bridges the injected `/mcp/agents` endpoint — it fetches `tools/list` at session start, registers each allow-listed daemon tool as `paseo_<name>` via `pi.registerTool`, and forwards `tools/call` over stateless JSON-RPC. No `--mcp-config` is written for read-only launches.
